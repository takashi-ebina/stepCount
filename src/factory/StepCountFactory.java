 ...
 * {@link StepCountFactory#create(String, IfCommentPatternMatch)}の引数に生成するオブジェクトに紐づく拡張子を渡してください。
 * <p>
...
public class StepCountFactory {
	...
	public enum StepCountType {
		...
		/**
		 * <p>
		 * 拡張子に対応するクラス名（完全修飾子）の値を返却するメソッド
		 * <p>
		...
		public static String getByClassName(final String extension) throws IllegalArgumentException {
			return new EnumReverseLookup<>(StepCountType.class, StepCountType::getExtension).lookup(extension)
					.getClassName();
		}
		 ...
		 * <p>
		 * 拡張子から該当のEnumを取得する処理は{@link EnumReverseLookup#containsExtension(Object)}に移譲しています。
		 * 
		 * @param extension 拡張子
	...
	/**
	 * <p>
	 * 拡張子に対応するステップカウントオブジェクトを生成するメソッド
	 * 
	 * @param extension           拡張子
	 ...
	 */
	public static IfStepCount create(final String extension, IfCommentPatternMatch commentPatternMatch) throws Exception {
		...
		try {
			stepCountObj = (IfStepCount) Class.forName(className)
					.getDeclaredConstructor(IfCommentPatternMatch.class).newInstance(commentPatternMatch);
		} catch (Exception e) {
...
//	public static IfStepCount create (final String extension, IfCommentPatternMatch commentPatternMatch)　{
//		IfStepCount stepCountObj = null;
//		if ("java".equals(extension)) {
//			stepCountObj = new JavaStepCount(commentPatternMatch);
//		} else if ("cs".equals(extension)) {
//			stepCountObj = new CsStepCount(commentPatternMatch);
//		}
//		return stepCountObj;
//	}
}
